...
# ═══════════════════════════════════════════════════════════════════════════
SAMPLE_PAPERS = [
    {"id": "pagerank1998",  "title": "The PageRank Citation Ranking: Bringing Order to the Web",
     "authors": "Page, L.; Brin, S.", "year": 1998, "venue": "Stanford Tech Report",
     "refs": []},
    {"id": "barabasi1999", "title": "Emergence of Scaling in Random Networks",
     "authors": "Barabási, A.-L.; Albert, R.", "year": 1999, "venue": "Science",
     "refs": ["pagerank1998"]},
    {"id": "smallworld1998", "title": "Collective dynamics of 'small-world' networks",
     "authors": "Watts, D.J.; Strogatz, S.H.", "year": 1998, "venue": "Nature",
     "refs": []},
    {"id": "newman2001",   "title": "The structure of scientific collaboration networks",
     "authors": "Newman, M.E.J.", "year": 2001, "venue": "PNAS",
     "refs": ["barabasi1999", "smallworld1998"]},
    {"id": "girvan2002",   "title": "Community structure in social and biological networks",
     "authors": "Girvan, M.; Newman, M.E.J.", "year": 2002, "venue": "PNAS",
     "refs": ["newman2001", "smallworld1998", "barabasi1999"]},
    {"id": "blondel2008",  "title": "Fast unfolding of communities in large networks",
     "authors": "Blondel, V.D. et al.", "year": 2008, "venue": "J. Stat. Mech.",
     "refs": ["girvan2002", "newman2001"]},
    {"id": "fortunato2010","title": "Community detection in graphs",
     "authors": "Fortunato, S.", "year": 2010, "venue": "Physics Reports",
     "refs": ["blondel2008", "girvan2002", "newman2001", "smallworld1998"]},
    {"id": "attention2017","title": "Attention Is All You Need",
     "authors": "Vaswani, A. et al.", "year": 2017, "venue": "NeurIPS",
     "refs": []},
    {"id": "bert2019",     "title": "BERT: Pre-training of Deep Bidirectional Transformers",
     "authors": "Devlin, J. et al.", "year": 2019, "venue": "NAACL",
     "refs": ["attention2017"]},
    {"id": "gpt32020",     "title": "Language Models are Few-Shot Learners",
     "authors": "Brown, T. et al.", "year": 2020, "venue": "NeurIPS",
     "refs": ["attention2017", "bert2019"]},
]
...
def load_sample():
    ...
    st.session_state["log"].append(f"✅ Loaded {len(SAMPLE_PAPERS)} sample papers.")
...
if page == "📊 Dashboard":
    ...
    with col_l:
        ...
        if st.button("🚀 Load Demo Corpus (10 papers)", use_container_width=True):
            load_sample()
            st.rerun()
    ...
    if not HAS_PYVIS:
        ...
        with col_cfg3:
            max_nodes = st.slider("Max nodes to display", 5,
                                  ...
                                  min(20, max(G.number_of_nodes(), 5)))
        ...
        if st.button("🔄 Render Graph"):
            ...
            for node in sub.nodes():
                ...
                size = 12 + 30 * (pr.get(node, 0) / pr_max)
                label = node if show_labels else ""
                tip = (f"<b>{art.get('title','?')[:60]}</b><br>"
                       f"{art.get('authors','')[:40]}<br>"
                       f"{art.get('year','')} · {art.get('venue','')}")
                net.add_node(node, label=label, title=tip, size=size,
                             color="#4fc3f7", borderWidth=2)
            for src, tgt in sub.edges():
                net.add_edge(src, tgt, color="rgba(79,195,247,0.35)", arrows="to")
            import tempfile, os
            tmp_path = os.path.join(tempfile.gettempdir(), "citation_graph_pyvis.html")
            net.save_graph(tmp_path)
            with open(tmp_path, "r", encoding="utf-8") as f:
                html_out = f.read()
            components.html(html_out, height=540, scrolling=False)
            st.caption(f"Showing top {max_nodes} nodes by degree. Node size ∝ PageRank.")
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    **Suggested workflow:**
    ...
    import requests
    r = requests.get(f"https://api.semanticscholar.org/graph/v1/paper/search"
